...
def music_list(request):
    ...
    serializer = MusicSerializer(musics, many=True) # serializer는 musics라고 하는 queryset을 json 타입으로 바꿔준다.
...
def comments_create(request, music_pk):
    ...
    if serializer.is_valid(raise_exception=True):
        # raise ValidationError(serializer.errors)  # raise_exception=True 로 이 부분 생략 가능, if 문도 validation을 통과 못 했을 경우 또한 고려하지 않아도 된다.
